...
- Core domain flow: a `Trip` belongs to a `BusModel`, `Route`, driver, and conductor; it accumulates `PassengerCount`/`Ticket` records and eventually attaches to a `Remittance` (a bus/crew's cash turnover for a period), which owner/audit-teller review and approve (`Remittance.approved_by`) and which can carry `RemittanceExpense` line items. Each `BusModel` has a current `assignedDriver`/`assignedConductor` plus a `BusCrewHistory` log of past crew assignments; `FareSettings` holds global fare config alongside per-route `FareRate` records.
