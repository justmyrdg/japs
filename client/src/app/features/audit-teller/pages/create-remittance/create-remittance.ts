...
@Component({
  selector: 'app-create-remittance',
  imports: [ReactiveFormsModule, CurrencyPipe, DatePipe],
  templateUrl: './create-remittance.html',
  styleUrl: './create-remittance.css',
})
export class CreateRemittancePage implements OnInit {
  ...
  loadBuses(): void {
    this.http.get<any[]>(`${this.API}/buses`, { withCredentials: true }).subscribe({
  ...
  onBusChange(e: Event): void {
    ...
    const bus = id ? (this.buses().find((b) => b.id === parseInt(id)) ?? null) : null;
  ...
  toggleAll(): void {
    if (this.selectedTripIds().size === this.allTrips().length) {
      this.selectedTripIds.set(new Set());
    } else {
      this.selectedTripIds.set(new Set(this.allTrips().map((t) => t.id)));
    }
    this.autoCalculateCommissions();
  }
  ...
  private buildExpensesPayload(): { expense_type: string; amount: number }[] {
  ...
  submitRemittance(): void {
    ...
    const bus = this.selectedBus()!;
    // Get driver_id and conductor_id from the first trip
    ...
    const payload = {
      ...
      conductor_id: firstTrip.conductor_id,
      date: this.selectedDate(),
      trip_ids: trips.map((t) => t.id),
      expenses: this.buildExpensesPayload(),
    };

    this.http.post<any>(`${this.API}/remittances`, payload, { withCredentials: true }).subscribe({
      next: () => {
        this.isSubmitting.set(false);
        this.alertService.success('Success', 'Remittance report submitted successfully.');
      ...
      error: (err) => {
        ...
        this.alertService.error(
          'Error',
          err.error?.message ?? 'Failed to submit remittance report.',
        );
      },
    });
  }
